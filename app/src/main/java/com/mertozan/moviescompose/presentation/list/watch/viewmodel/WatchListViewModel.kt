...
class WatchListViewModel @Inject constructor(
    ...
    fun onAction(action: WatchListAction) {
        when (action) {
            ...
            is WatchListAction.RemoveContentFromWatched -> removeFromWatchedList(
                action.id,
                action.isWatched,
                action.listType,
                action.type
    ...
    private fun removeFromWatchedList(id: Int, isWatch: Boolean, listType: String, type: String) {
        when (listType) {
            ListType.POPULAR.name -> when (type) {
                ContentType.SERIES.name -> updateSeriesWatched(id, isWatch)
                ContentType.MOVIE.name -> updateMovieWatched(id, isWatch)
            }

            ListType.TOP_RATED.name -> when (type) {
                ContentType.SERIES.name -> updateTopSeriesWatch(id, isWatch)
                ContentType.MOVIE.name -> updateTopMovieWatched(id, isWatch)
            }
        }
    }
